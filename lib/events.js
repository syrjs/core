  ...
  constructor() {
    ...
    const fireLifeCycle = Utils.debounce(function(notifiers) {
      for (var notifier in notifiers) {
        if (notifiers.hasOwnProperty(notifier)) {
          bubbleEvent(notifiers[notifier].event, notifiers[notifier].component);
        }
    }
    }, 100);
    ...
    function handleLifeCycle(event, component) {
      let walk = (event, component) => {
        ...
        } else if (handler && !component.parent) {
          lifeCycleNotifiers[component.guid] = {component: component, event: event};
        }
      }
