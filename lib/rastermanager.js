    ...
    ast.children.forEach(function (child, index) {
        if (typeof child !== 'string' && typeof child !== 'number') {
          ...
          if(child.attributes && child.attributes.key) {
            child.uuid = child.uuid + '-' + child.attributes.key;
...
const updateComponent = (component, rendered) => {
  ...
  function reflow(component, passedUpdates) {
    ...
    if(component.children) {
      ...
      if(component.uuid !== component.guid) {
        let guidPos  = component.uuid.indexOf(component.guid);
        let fenceid = component.uuid.substr(0, guidPos);
        console.log('woot',fenceid)
      }
      ...
      component.children.forEach((child, index)=>{
        if(Array.isArray(updates[index])) {
          updates = flattenChildren(updates);
        }
