  ...
  function reflow(component, passedUpdates) {
    ...
    if (updates.length > 0) {
      ...
      if (updates.length > component.children.length) {
        ...
        console.log('removales to be made', JSON.stringify(component.children), updates);
      }
      ...
      component.children.forEach((child, index) => {
        ...
        if (child.unmount) {
          ...
          if (child.elementName == (update && update.elementName && update.elementName.name)) {
            // the types are the same, lets update
            delete child.unmount;
            ...
            if(!unmount) {
              ...
              //deleting cache of updates with new replacement only after removing the component from the list of children
          ...
          //unmounting the old component in the same position of the parent.
          let unmount = updateIDs[child.instance.uuid];
          if (!unmount) {
            // child not found in update so lets unmount it
            child.unmount = true;
            //not deleting cache yet, this will cause mount issues.
            //this delete will happen on the next cycle
          }
        } else if (update) {
          ...
          if (child.elementName == update.elementName.name) {
            // the types are the same, lets update
            if(child.guid == update.guid) reflow(child, update);
          } else {
            ...
            let updatedElement =  _cache[`${fenceid}-${update.guid}`];
            delete updatedElement.unmount;
            _cache[`${fenceid}-${update.guid}`] = updatedElement;
            reflow(updatedElement, update);
          }
        } else {
      ...
      if (lastIndex < updates.length - 1) {
        for (let i = lastIndex + 1; i <= updates.length - 1; i++) {
          ...
          } else if (updates[i] && updates[i].new) {
