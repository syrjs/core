  ...
  if (ast.children) {
    ast.children.forEach(function(element) {
      if (typeof element.elementName == 'function') {
        ...
        if (element.elementName == 'HTMLImageElement') {
          element.elementName = 'Image';
        }
        // pass down props
        ...
        if (element.elementName == 'Button') {
          ...
          element.instance.props.onPress = element.instance.props.onPress.bind(
            ast.instance
          );
        ...
        if (element.children) {
          element.children.forEach(subelement => {
            if (typeof subelement == 'function') {
              subelement.instance = new subelement.elementName();
              subelement.elementName = subelement.instance.constructor.name;

              if (subelement.elementName == 'HTMLImageElement') {
                subelement.elementName == 'Image';
              }
              subelement.instance.props = subelement.attributes;
              subelement.instance.elementName = subelement.elementName;
              subelement.children =
                (subelement.instance.render && subelement.instance.render()) ||
                subelement.children;
              subelement.parent = element.instance;
              Events.register(subelement.instance); // this is ugly
              console.log('subelement', subelement);
            } else {
              // return as value
            }

            console.log('hi', subelement);
            // create instances of the children
            ...
            // subelement.children = (subelement.instance.render && subelement.instance.render()) || subelement.children ;
