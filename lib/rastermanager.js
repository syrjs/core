  ...
  function reflow(component, passedUpdates) {
    ...
    if(component.children) {
      ...
      if(updates.length > component.children.length) {
        ...
        // console.log('insertions to be made', component.children[0].fenceid);
      ...
      // this is a dumb check, should actually verify types are the same
      component.children.forEach((child, index)=>{
        if(updates[index]) {
          if (child.elementName == updates[index].elementName.name) {
            // the types are the same, lets update
            reflow(child, updates[index]);
          } else {
            if(updates[index].new) {
              let newComponent = createComponent(updates[index]);
              component.children.splice( index, 0, newComponent );
              console.log('need to make a new element', updates[index], newComponent)
            } else {
              console.log('type mismatch', child, updates[index], 'at index: '+ index);
            }
          }
        }
      });
